...
function runCli(args: string[], cwd: string, home: string): RunResult {
...
function makeDirs(): { cwd: string; home: string } {
...
test('zip mode pairs array values positionally', () => {
  const { cwd, home } = makeDirs();
  const res = runCli(
    ['Review {{file}} for {{focus}}', '--file=a.go,b.go', '--focus=x,y', '--no-interactive'],
    cwd,
    home,
  );
  assert.strictEqual(res.status, 0);
...
test('--cross produces the full cross-product of array values', () => {
  const { cwd, home } = makeDirs();
  const res = runCli(
    ['Review {{file}} for {{focus}}', '--file=a.go,b.go', '--focus=x,y', '--cross', '--no-interactive'],
    cwd,
    home,
  );
  assert.strictEqual(res.status, 0);
...
test('--status prints only the status line for the first iteration', () => {
  const { cwd, home } = makeDirs();
  const res = runCli(
    ['Review {{file}} for {{focus=bugs}}', '--file=a.go', '--status', '--no-interactive'],
    cwd,
    home,
  );
  assert.strictEqual(res.status, 0);
...
test('defaults fill unset variables in non-interactive mode', () => {
  const { cwd, home } = makeDirs();
  const res = runCli(
    ['Be {{tone=concise}} about {{file}}', '--file=x.ts', '--no-interactive'],
    cwd,
    home,
  );
  assert.strictEqual(res.status, 0);
...
test('reserved control flags never fill same-named template variables', () => {
  const { cwd, home } = makeDirs();
  const res = runCli(
    ['Mode {{cross=zip}} status {{status=ok}}', '--cross', '--status', '--no-interactive'],
    cwd,
    home,
  );
  assert.strictEqual(res.status, 0);
